...
export default function LoginModal({
  ...
  const [errors, setErrors] = useState({ email: "", username: "", password: "", confirmPassword: "", otp: "" })
  const [phase, setPhase] = useState("form") // form | otp | loading | success | failed
  ...
  const isCloseDisabled = phase === "loading" || phase === "otp"

  function validate() {
    const newErrors = { email: "", username: "", password: "", confirmPassword: "", otp: "" }
  ...
  function handleFailedRegistration() {
    ...
    setTimeout(() => {
      setPhase("failed")
      setIsLoading(false)
      toast.error("Registration failed: OTP verification required", { duration: 3000 })
    }, 2000)
  }
  ...
  function handleBackToForm() {
    setPhase("form")
    setErrors({ email: "", username: "", password: "", confirmPassword: "", otp: "" })
    setOtp("")
  }
  ...
  return (
    <>
          ...
          <div className="bg-gradient-to-br from-white via-slate-50 to-slate-100 p-6">
            <DialogHeader>
              <DialogTitle className="text-2xl font-semibold tracking-tight text-slate-900">
                {isRegisterMode ? "Create Your Account" : title}
              </DialogTitle>
              {description && (
                <DialogDescription className="text-sm text-slate-600">
                  {isRegisterMode ? "Fill in the details to register a new account" : description}
                </DialogDescription>
              )}
            </DialogHeader>
          </div>
                    ...
                    >
                      {isLoading ? (
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {isRegisterMode ? "Preparing OTP..." : "Signing In..."}
                        </div>
                      ) : (
              ...
              {phase === "loading" && (
                ...
                >
                  <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50 w-full">
                    <div className="flex items-center gap-3 p-4">
                      ...
                      <span className="text-sm text-slate-700">
                        {isRegisterMode ? "Processing registration..." : "Authenticating your credentials..."}
                      </span>
                    </div>
                  ...
                  <div className="text-sm text-slate-600">
                    Please wait while we {isRegisterMode ? "process your registration" : "verify your account"}...
                  </div>
                </motion.div>
              )}
      ...
      <Dialog open={showOtpModal && phase === "otp"} onOpenChange={handleOtpModalClose}>
